...
const GovernanceReport: React.FunctionComponent<IWidgetProps> = (props) => {
    ...
    const fetchDocuments = () => {
        setLoading(true);
        props.uxpContext?.executeAction('ESG', 'showGovernanceReportDocuments', { selectedYear}, {json:true})
            .then((res) => {
                setDocuments(res);
            })
            .catch((error) => {
    ...
    const handleDownloadPDF = async (doc: Document) => {
        try {
            ...
            const reportData = await props.uxpContext?.executeAction(
                "ESG", 
                "downloadGovernanceTransactionReport",  // API to fetch transaction report
            ...
            const pdf = new jsPDF();
            pdf.setFontSize(16);
            pdf.text("Governance Report", 105, 20, { align: "center" });
            ...
            (pdf as any).autoTable({
                startY: 30,
                head: [["Activity ID", "ActivityGroup", "ActivityCategory", "Year", "Unit","Value"]],
                body: tableData,
            });
    
            // Download PDF
            pdf.save(`Governance_Report_${doc.Year}_${doc.TransactionID}.pdf`);
    ...
    const handleDownloadXLSX = async (doc: Document) => {
        try {
            ...
            // Download Excel file
            XLSX.writeFile(workbook, `Governance_Report_${doc.Year}_${doc.TransactionID}.xlsx`);
        } catch (error) {
            alert.show("Failed to download report");
            console.error("Error generating report:", error);
        }
    ...
    return (
        <WidgetWrapper className="social-approval-widget">
            <TitleBar title='Governance Reports' className="title-bar">
                <FilterPanel onClear={() => {
                    setSelectedYear("");
                }}>
                    <FormField>
